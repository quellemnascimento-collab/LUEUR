# LUEUR — Plano de Negócio Consolidado (v2)
### Versão ajustada: combina o plano original LUEUR com o material recebido, corrigindo inconsistências

> Este documento substitui o `PLANO_DE_NEGOCIO.md` como referência principal. Mantém o conceito autoral da marca
> (diferenciação real) e incorpora os dados de mercado, concorrentes, fornecedores e canais do material recebido —
> com ajustes nas projeções financeiras, que estavam infladas e inconsistentes no material original.

---

## 0. O que mudou nesta versão — Resumo das diferenças

| Item | Plano original LUEUR | Material recebido | **Ajuste final (v2)** |
|---|---|---|---|
| Conceito de marca | "Fases da Luz" (Aurora/Golden Hour/Dusk/Night Glow), estética noturna | "Iluminação criativa", minimalismo escandinavo | **Mantém "Fases da Luz"** (mais original e defensável) + adota paleta clara/aconchegante do material recebido como base do dia a dia, reservando tons escuros/Night Glow como assinatura de diferenciação |
| Paleta | Predominantemente escura (preto, vinho, azul-noite) | Predominantemente clara (creme, dourado, rosa pôr-do-sol) | **Paleta híbrida**: base clara/quente (creme aurora, âmbar, bronze) para 80% da comunicação + "Night Glow" como linha cápsula de alto impacto visual |
| Concorrentes | Focava só em marcas de nicho/design (Tropix, Else, Brink) | Inclui players de massa (Kalunga, Tilibra, Faber-Castell) e nicho (Papel de Pato, Planeta Kawaii) | **Mescla as duas listas** — investidor precisa ver o cenário completo (massa + nicho + premium) |
| Marketplaces | Shopee, ML, Elo7, Amazon, Instagram/TikTok Shop | Adiciona Magalu, Americanas, hub integrador | **Inclui todos**, mas prioriza 2 no início (ver seção 6) |
| Eventos | Genéricos | Nomes e datas específicas (Escritório Escolar Brasil, EMBANEWS, LicenciamentoCon) | **Mantém os nomes específicos**, com aviso para confirmar datas/local antes de planejar viagem/orçamento |
| Fornecedores | Categorias genéricas | Lista nominal com contatos (Reval, Jandaia, JC Distribuição etc.) | **Mantém a lista como ponto de partida de pesquisa** — contatos/telefones devem ser confirmados, pois não há garantia de exatidão |
| **Projeções financeiras** | Conservadoras, baseadas em operação enxuta (R$32k/mês no mês 6) | Agressivas: R$200k/mês no mês 12, lucro de R$328k no ano 1, pedido de R$350k a investidor | **Corrigido** — ver seção 8. Mantém a estrutura de "rodada de investimento", mas com números plausíveis e cenários (conservador/moderado/agressivo) |

---

## 1. Visão Executiva

**LUEUR** ocupa o espaço entre três zonas do mercado brasileiro de papelaria:

- **Papelaria de massa** (Kalunga, Tilibra) — preço baixo, sem identidade emocional.
- **Papelaria kawaii/fofa** (Planeta Kawaii, Bailarina de Papel) — forte em nicho, mas estética saturada.
- **Premium importado** (Muji, Kokuyo, Staedtler) — preço alto, sem conexão local/emocional com o consumidor brasileiro.

**Proposta de valor LUEUR**: *"Papelaria de autor que transforma o ritual de organizar e criar em uma experiência
de luz própria — minimalismo acolhedor no dia a dia, com uma linha de assinatura (Night Glow) que literalmente
brilha."*

- **Ticket médio-alvo**: R$ 75–110 (posicionamento "premium acessível", entre a massa de R$30-50 e o importado de R$300+).
- **Canal principal**: e-commerce próprio + marketplaces selecionados + redes sociais como motor de marca.

---

## 2. Mercado — Dados de Contexto

> *Os números abaixo vêm do material recebido. Não foi possível verificar as fontes originais nesta sessão —
> recomenda-se confirmar com relatórios públicos (ex: Sebrae, ABRELIVROS, Statista) antes de usar em pitch formal
> a investidores, citando a fonte exata.*

| Indicador | Valor referenciado |
|---|---|
| Mercado global de papelaria (2025) | ~US$ 114 bilhões |
| Projeção 2032 | ~US$ 160 bilhões (CAGR ~5%) |
| Crescimento e-commerce papelaria Brasil (2025) | ~20% (acima do varejo geral) |
| Crescimento transações livrarias/papelarias (jan/2025) | ~16% |

**Leitura estratégica**: independentemente da precisão exata dos números, a tendência é consistente com o que
observamos no comportamento de consumo (crescimento de e-commerce de nicho, comunidades de planner/journaling,
busca por produtos com identidade). Isso sustenta o racional de mercado sem depender de um número específico.

---

## 3. Concorrentes — Mapa Completo

### 3.1 Players de massa (referência de escala, não de estética)

| Marca | Posicionamento | Por que está no radar |
|---|---|---|
| **Kalunga** | Maior rede de papelaria do Brasil (200+ lojas + e-commerce) | Referência de distribuição e preço-piso; LUEUR não compete em preço, compete em experiência |
| **Tilibra** | Fabricante, forte em material escolar | Referência de portfólio amplo e fabricação própria — possível parceiro de fabricação a longo prazo |
| **Faber-Castell** | Escrita premium, marca alemã global | Benchmark de qualidade de produto (canetas/lápis) |

### 3.2 Players de nicho/design (concorrência direta de posicionamento)

| Marca | Diferencial | Onde LUEUR se diferencia |
|---|---|---|
| **Papel de Pato** (@duckpaper) | Produtos exclusivos, comunidade digital forte desde 2022 | LUEUR aposta no sistema de coleções "Fases da Luz" (recorrência/colecionismo) e na linha Night Glow (produto fisicamente diferenciado) |
| **Planeta Kawaii** / **Bailarina de Papel** | Estética kawaii/fofa, kits temáticos | LUEUR foge do kawaii — minimalismo acolhedor + drop "Night Glow" como contraponto sofisticado |
| **Newpen** | Canetas premium + personagem/mascote ("A Patroa") | LUEUR usa um símbolo abstrato de luz (não personagem fofo) — fica mais "premium acessível" e menos infantilizado |
| **Planejador Fabuloso** | 100% digital (planners virtuais) | Oportunidade: LUEUR pode oferecer versão física + digital complementar (ex: PDF de adesivos digitais incluso) |

### 3.3 Premium internacional (benchmark de produto/estética)

MUJI, Kokuyo, Staedtler, Pilot, Zebra — usados como referência de **qualidade de papel, acabamento e design
funcional**, não como concorrência direta (preço/distribuição muito diferentes).

### Gap de mercado confirmado

Não há, hoje, uma marca brasileira que combine: **(1)** sistema de coleções temáticas com lógica emocional clara
(Fases da Luz), **(2)** um produto-assinatura tecnicamente diferenciado (tinta fosforescente Night Glow) e
**(3)** estética "minimalismo acolhedor" (nem kawaii, nem corporativo). Esse é o espaço da LUEUR.

---

## 4. Branding — Versão Ajustada

### 4.1 Conceito central

> **"Cada um brilha na sua hora."** — sistema "Fases da Luz" (Aurora, Golden Hour, Dusk, Night Glow), mantido do
> plano original por ser mais distintivo que o conceito genérico de "iluminação criativa".

### 4.2 Paleta de cores (revisada — híbrida)

A paleta original (excesso de preto/escuro) foi ajustada para um **uso 80/20**: base clara e quente para o dia a
dia da marca (feed, embalagens padrão, site), com a paleta escura reservada para a linha **Night Glow** — isso
torna a marca mais convidativa no Instagram/Pinterest sem perder o diferencial noturno.

| Cor | Nome | Uso | Origem |
|---|---|---|---|
| `#FFF8F0` | Creme Aurora | Fundo principal, embalagens | material recebido |
| `#D4A574` | Âmbar Quente | Textos, logotipo, detalhes | material recebido |
| `#8B6F47` | Bronze da Terra | Acabamentos premium (hot stamping) | material recebido |
| `#FFB4A0` | Rosa do Pôr do Sol | Acentos da coleção Aurora | material recebido |
| `#2C241B` | Tinta da Meia-Noite | Tipografia principal, linha Night Glow | material recebido |
| **+ Detalhe fosforescente/prata** | "Glow" | Exclusivo da linha Night Glow (capas, adesivos) | plano original LUEUR — **diferencial técnico mantido** |

### 4.3 Tipografia e logotipo

- **Títulos**: Playfair Display (serifada premium) — sugestão do material recebido, mais acessível/legível que a
  opção anterior (Fraunces/Canela), mantendo elegância editorial.
- **Corpo/UX**: Inter ou Lato.
- **Logotipo**: wordmark "LUEUR" com um glifo de luz acima do "U" — pode ser o "ponto de luz" do plano original
  **ou** a estrela de 4 pontas (✦) sugerida no material recebido. Recomendação: testar as duas versões com o
  público-alvo (enquete no Instagram) antes de fechar — decisão de baixo custo e alto impacto de identidade.
- **Mascote**: **não usar mascote/personagem fofo** (consenso entre os dois materiais) — manter identidade
  abstrata baseada em luz/gradiente.

### 4.4 Tom de voz

Consolidado: **sofisticado, acolhedor, calmo** — evita tanto o "kawaii excessivo" quanto a "motivação rasa".
Vocabulário-chave: luz, claridade, ritual, brilho próprio, organização sem estresse.

---

## 5. Linguagem para Redes Sociais — Consolidado

### 5.1 Públicos-alvo (mesclado)

| Segmento | Perfil | Necessidade |
|---|---|---|
| Profissionais criativos | 25-40 anos, designers, redatoras, educadoras | Organização + estética + funcionalidade |
| Estudantes (vestibular/pós) | 18-30 anos | Planejamento + bem-estar + produtividade |
| Empreendedoras | 28-45 anos | Controle do negócio + autocuidado |
| Planejadores conscientes | 30-50 anos | Organização sem estresse + sustentabilidade |

### 5.2 Frequência de postagem (referência prática)

| Rede | Frequência |
|---|---|
| Instagram | 4-5 posts/semana + Stories diários + 2-3 Reels/semana |
| Pinterest | 10-15 pins/semana (alto retorno de SEO visual para papelaria) |
| TikTok | 3-5 vídeos/semana (ASMR, bastidores, trends) |
| YouTube | 1 vídeo/mês (formato longo: "um dia com LUEUR", reviews) |
| LinkedIn | 2 posts/mês — relevante apenas quando houver linha B2B (vendas corporativas/personalizadas) |

### 5.3 Exemplos de copy (adaptados ao tom LUEUR)

> **Lançamento Night Glow**: "Tem caderno que você guarda na gaveta. Tem caderno que brilha de volta pra você no
> escuro. Bem-vinda à fase Night Glow — porque sua melhor ideia pode chegar às 23h. ✦🌙"

> **Coleção Aurora**: "Planejar não é prender sua rotina — é abrir espaço pra ela respirar. Nova fase Aurora: para
> começar o dia com clareza, não com pressa."

> **Sustentabilidade**: "Papel reflorestado, tintas à base de água, embalagens que voltam pra natureza. Brilho que
> não custa o futuro. 🌱✦"

### 5.4 Conteúdos essenciais (lista consolidada)

Unboxing ASMR, "setup luminoso" (mesa de trabalho), antes/depois de organização, tutoriais de uso de planner,
bastidores de produção, depoimentos de clientes, comparativos de canetas/materiais, "qual fase da luz é você?"
(quiz interativo).

---

## 6. Marketplaces — Lista Completa e Priorização

### 6.1 Todos os canais mapeados

| Canal | Prioridade | Taxas aproximadas | Observação |
|---|---|---|---|
| **Site próprio (Nuvemshop)** | Imediata | taxa de gateway de pagamento (~3-5%) | Recomendado pela integração nativa com ML/Shopee/PagSeguro |
| **Shopee** | Imediata | ~14-20% | Tráfego jovem, ótimo para itens de ticket menor |
| **Mercado Livre** | Imediata/Mês 2 | ~12-19% | Maior alcance, exige boa reputação para escalar |
| **Elo7** | Mês 2-3 | ~13% ou R$29,90/mês (plano básico) | Nicho de papelaria criativa/artesanal — bom para produtos exclusivos |
| **Instagram/TikTok Shop** | Mês 1-2 | taxa de gateway | Vitrine + venda direta no funil de conteúdo |
| **Magalu** | Mês 4-6 (expansão) | ~12-18% | Exige CNPJ + documentos empresariais completos |
| **Americanas** | Mês 4-6 (expansão) | ~13-19% | Alto tráfego, exige operação logística madura |
| **Amazon BR** | Mês 6+ (expansão) | ~15-17% (+ logística se FBA) | Público "premium", exige fotos/especificações muito completas |

### 6.2 Priorização recomendada (realista para operação enxuta)

- **Meses 1-3**: Site próprio + Shopee + Instagram/TikTok Shop (menor complexidade, cadastro rápido, CPF aceito
  na Shopee inicialmente).
- **Meses 4-6**: adicionar Mercado Livre (com CNPJ já emitido) + Elo7.
- **Meses 7+**: avaliar Magalu/Americanas/Amazon **somente se** a operação logística já estiver estável
  (entregas sem atraso > 2 dias) — esses canais exigem documentação empresarial completa e penalizam atrasos.

### 6.3 Passo a passo resumido por canal

- **Mercado Livre**: mercadolivre.com.br/vender → cadastro (CPF inicial ou CNPJ) → enviar documentos → configurar
  Mercado Pago + Mercado Envios → cadastrar produtos com fotos HD e título com palavras-chave.
- **Shopee**: shopee.com.br/vendedor → registrar como vendedor → CPF ou CNPJ (CNPJ reduz taxas) → configurar conta
  bancária e frete (Shopee Xpress) → cadastrar produtos com foto + vídeo.
- **Elo7**: elo7.com.br → "Vender no Elo7" → CPF/CNPJ → escolher plano (mensal fixo vs. comissão) → montar vitrine.
- **Magalu/Americanas/Amazon**: exigem CNPJ + documentos empresariais (contrato social, certidões) — planejar
  esse cadastro **somente após a migração para ME** (ver `ESTRUTURA_EMPRESARIAL.md`).

### 6.4 Hub integrador de estoque

Recomendado a partir do momento em que a LUEUR estiver em **3+ canais simultâneos** — evita overselling.
Opções: Bling/Tiny (mencionados no plano anterior) ou integradores como "Integra Online" (citado no material
recebido). Custo médio: R$ 100–300/mês no plano inicial. **Não é prioridade nos primeiros 2 meses.**

---

## 7. Eventos do Nicho — Lista Consolidada

| Evento | Data referenciada | Local | Por que importa |
|---|---|---|---|
| **Escritório Escolar Brasil** | 03–06/ago (referência 2025) | São Paulo - SP | Citado como maior evento do setor na América Latina — prioridade nº 1 para visita |
| **EMBANEWS Eventos** | 26–29/mai (referência 2026) | São Paulo - SP | Foco em papelaria/material escolar |
| **LicenciamentoCon** | 27–28/ago (referência 2025) | São Paulo - SP | Licenciamento de marcas/personagens — relevante se LUEUR fizer colaborações de marca |

> ⚠️ **Importante**: confirme as datas e locais exatos diretamente nos sites oficiais dos eventos antes de
> reservar orçamento/viagem — essas datas vieram do material recebido e não foram verificadas nesta sessão.

**Estratégia de participação (mantida do plano original)**: Ano 1 — apenas visitar (fornecedores/tendências);
Ano 2 — estande pequeno; Ano 3 — estande premium com lançamentos exclusivos.

---

## 8. Projeções Financeiras — Versão Corrigida

> O material recebido projetava R$ 200.000/mês de faturamento já no mês 12 e R$ 954.000 no ano 1, com lucro
> líquido de R$ 328.000 (margem de 34%) — **esses números assumem um ritmo de crescimento (dobrar a receita a
> cada 1-2 meses por 12 meses seguidos) que normalmente só ocorre com aporte de capital relevante e investimento
> pesado em mídia paga desde o mês 1.** Abaixo, apresentamos **3 cenários** para que a decisão (e a conversa com
> investidores) seja feita com transparência sobre as premissas.

### 8.1 Cenário Conservador (bootstrap — sem aporte externo)

Baseado no `ESTRUTURA_EMPRESARIAL.md`: crescimento orgânico, reinvestimento do próprio caixa, 1-2 contratações
no semestre.

| Mês | Faturamento (R$) |
|---|---|
| 1 | 6.000 |
| 2 | 9.000 |
| 3 | 13.000 |
| 6 | 32.000 |
| 12 (estimado, mantendo ritmo de ~20-25% a.m. com desaceleração natural) | 70.000 – 90.000 |

- **Faturamento ano 1 estimado**: R$ 350.000 – 450.000.
- **Necessidade de caixa**: R$ 65-90k (conforme já calculado).

### 8.2 Cenário Moderado (com aporte parcial — ex: R$ 100-150k)

Permite: compra de estoque maior desde o início (reduz custo unitário por volume), tráfego pago consistente
(R$ 3-5k/mês desde o mês 2), e contratação da equipe básica (logística + social media) já no mês 2-3.

| Mês | Faturamento (R$) |
|---|---|
| 1 | 10.000 |
| 3 | 25.000 |
| 6 | 55.000 |
| 9 | 90.000 |
| 12 | 130.000 |

- **Faturamento ano 1 estimado**: R$ 600.000 – 750.000.
- **Margem líquida realista**: 15-25% no ano 1 (ainda absorvendo custos de estruturação), podendo chegar a
  30-35% no ano 2 com operação madura.

### 8.3 Cenário Agressivo (aporte robusto — R$ 300-350k, conforme proposta do material recebido)

Mantém a **estrutura de uso de capital** sugerida (estoque, e-commerce/branding, marketing, operacional, reserva),
mas com metas de faturamento **revisadas para um patamar alcançável**:

| Uso do capital | % | Valor (sobre R$ 350k) |
|---|---|---|
| Estoque inicial (volume maior, melhores preços de fornecedor) | 35% | R$ 122.500 |
| E-commerce + branding + fotografia | 12% | R$ 42.000 |
| Marketing + tráfego pago + influenciadores | 25% | R$ 87.500 |
| Operacional (equipe, logística, contabilidade) | 18% | R$ 63.000 |
| Eventos do setor (1 grande evento/ano) | 5% | R$ 17.500 |
| Reserva de emergência | 5% | R$ 17.500 |

| Mês | Faturamento (R$) |
|---|---|
| 3 | 35.000 |
| 6 | 75.000 |
| 9 | 120.000 |
| 12 | 160.000 – 180.000 |

- **Faturamento ano 1 estimado**: R$ 850.000 – 1.000.000 (próximo ao número do material recebido, mas atingido
  de forma mais gradual — pico no mês 12, não sustentado desde o início).
- **Margem líquida ano 1**: 15-20% (realista para um ano de forte investimento em estruturação) — lucro líquido
  estimado entre **R$ 130.000 – 200.000** (abaixo dos R$328k originais, mas defensável perante investidor).

### 8.4 Recomendação para o pitch a investidores

- **Não apresente apenas o cenário agressivo como única projeção** — investidores experientes desconfiam de
  curvas de crescimento "perfeitas" (dobrar todo mês). Apresentar os **3 cenários com premissas explícitas**
  aumenta a credibilidade.
- **ROI e prazo de retorno**: com o cenário agressivo revisado (lucro ~R$130-200k no ano 1 sobre aporte de
  R$350k), o retorno integral do aporte ocorre de forma mais realista entre **18-24 meses**, não 9-11 meses como
  no material original. Isso é normal para varejo/D2C e ainda é um ROI atrativo.
- **Estrutura societária**: manter a proposta de 20-25% de participação para R$ 350k (valuation implícito de
  R$1,4-1,75 milhão) é razoável **se** acompanhada de metas (milestones) claras de uso de capital e marcos de
  faturamento — sugerir liberação do aporte em tranches (ex: 40% na assinatura, 30% ao atingir X faturamento
  mensal, 30% ao abrir o 4º canal de venda).

---

## 9. Fornecedores — Lista Consolidada

### 9.1 Fornecedores nacionais (ponto de partida — confirmar contatos)

| Fornecedor | Especialização | Local |
|---|---|---|
| Reval | Distribuidora multimarcas | São Paulo - SP |
| Jandaia Atacado (Grupo Bignardi) | Fabricante de cadernos | São Paulo - SP |
| Faber-Castell (compras corporativas) | Canetas/lápis premium | São Paulo - SP |
| JC Distribuição | Multicategoria | Goiás |
| Wespi Atacado | Escritório/informática/papelaria | Maringá - PR |
| Astral Atacado | Multimarcas | Belo Horizonte - MG |

> ⚠️ Os contatos (telefones/sites) listados no material recebido **não foram verificados** nesta sessão — antes
> de fechar pedidos, confirme CNPJ ativo, condições comerciais e referências de outros clientes.

### 9.2 Fornecedores para a linha Night Glow (diferencial técnico — mantido do plano original)

- Fornecedores de **tinta/verniz fosforescente** para serigrafia (pesquisar fornecedores de insumos gráficos
  especializados — esse insumo não consta no material recebido e é o principal diferencial técnico da marca).
- Distribuidoras de **papel especial** (perolado, holográfico fosco, kraft).

### 9.3 Lista de estoque inicial (referência de quantidades — ajustar ao orçamento disponível)

| Categoria | Itens | Qtd inicial sugerida | Observação |
|---|---|---|---|
| Canetas | Esferográficas, gel, marcador | 300-500 (não 1000 — ajustar ao cenário conservador) | Iniciar com curadoria menor, validar giro antes de escalar |
| Cadernos/Planners ("Fases da Luz") | A5, capa especial | 100-150 por coleção | Conforme já detalhado no `PLANO_DE_NEGOCIO.md` original |
| Adesivos (incl. Night Glow) | Folhas variadas | 300-500 | Produto de entrada, alto giro |
| Acessórios (clipes, marcadores, elásticos) | Mix curado | 200-400 | Revenda inicial, marca própria a médio prazo |

> A lista de 64k em estoque do material recebido (1000 canetas, 500 cadernos etc.) é compatível com o
> **cenário moderado/agressivo** (seção 8.2/8.3) — para o cenário conservador (bootstrap), reduzir
> proporcionalmente em ~60-70%.

---

## 10. Roadmap — 12 Meses (Consolidado)

| Fase | Meses | Marcos |
|---|---|---|
| **Fundação** | 1-2 | CNPJ/MEI, registro de marca (INPI), identidade visual final (paleta híbrida), 1ª produção (Aurora + Night Glow), cadastro fornecedores, site + Shopee + Instagram Shop |
| **Lançamento** | 3-4 | Campanha de lançamento, parcerias com microinfluenciadoras, cadastro Mercado Livre + Elo7, primeiros 100-300 clientes |
| **Crescimento** | 5-8 | Lançamento Golden Hour, expansão de catálogo, avaliação de migração MEI→ME, possível 1ª contratação (logística) |
| **Escala** | 9-12 | Avaliação de canais adicionais (Magalu/Amazon) **se** operação estável, participação em evento do setor (visita), avaliação de rodada de investimento com base em dados reais dos primeiros 9 meses |

---

## 11. Mensagem Final para Investidores (ajustada)

LUEUR ocupa um espaço real e validável entre a papelaria de massa, o kawaii saturado e o premium importado —
com um sistema de coleções ("Fases da Luz") que gera recorrência natural e um produto-assinatura tecnicamente
diferenciado (linha Night Glow, com tinta fosforescente).

Em vez de prometer uma curva de crescimento linear "perfeita", a LUEUR apresenta **3 cenários transparentes**:
um caminho de validação orgânica (bootstrap), um caminho de aceleração moderada e um caminho de escala com aporte
de R$ 350k — este último com retorno estimado em 18-24 meses, uso de capital detalhado e liberação em tranches
vinculadas a marcos de execução. Essa transparência é, em si, parte do diferencial da marca: **clareza também é
luz.**
